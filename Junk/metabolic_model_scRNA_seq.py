...
with open("Data/Input/nomenclature_files/to_hgnc_dictionary/to_hgnc_A549_genes.json", "r") as conversion_file:
...
for i in range(len(scRNAseq_gene_list)):
	try:
		...
		print(f"Could not find match in search for {scRNAseq_gene_list[i]}\n"
		      f"This seems to result from glitch in HGNC checker")
