...
# Generates files containing correct experimental fluxes for each condition.
# Not always a part of the workflow, only required due to having 8 conditions.
...
# Defines paths
# mass spec data location
mass_spec_path = Path("Data/experimental_alignment_data/recon_1_t_cells_12_11_23/Data_For_All_Mice/Expt_Met_Flux_Data.csv")
biomass_path = Path("Data/experimental_alignment_data/recon_1_t_cells_12_11_23/Data_For_All_Mice/Biomass_Data.csv")
cell_size_path = Path("Data/experimental_alignment_data/recon_1_t_cells_12_11_23/Data_For_All_Mice/cell_size.txt")
Allowed_Exchange_Reactions_path = Path("Data/experimental_alignment_data/recon_1_t_cells_12_11_23/Data_For_All_Mice/Allowed_Exchange_Reactions")
output_folder_path = Path("Data/experimental_alignment_data/recon_1_t_cells_12_11_23/Individual_Mice/")

# loads experimental flux data
ms_data = pd.read_csv(mass_spec_path)
bm_data = pd.read_csv(biomass_path)

mouse_number_filename_dict = {1:"B6-1",2:"B6-2",3:"B6-3",4:"B6-4",5:"TC-5",6:"TC-6",7:"TC-7",8:"TC-8"}

# creates conversion dict from named metabolites to exchange flux reactions
...
for mouse_number in mice_numbers:
	if not os.path.isdir(output_folder_path / mouse_number_filename_dict[mouse_number]):
		os.mkdir(output_folder_path / mouse_number_filename_dict[mouse_number])
	shutil.copy(cell_size_path,output_folder_path / mouse_number_filename_dict[mouse_number] / "cell_size.txt")
	shutil.copy(Allowed_Exchange_Reactions_path, output_folder_path / mouse_number_filename_dict[mouse_number] / "Allowed_Exchange_Reactions")
...
for mouse_number in mice_numbers:
	...
	output_dataframe.to_csv(output_folder_path / mouse_number_filename_dict[mouse_number] / "Measured_Rates.csv",
	                        index=False)
	
	output_dataframe_bm.to_csv(output_folder_path / mouse_number_filename_dict[mouse_number] / "Biomass_Data.csv",
	                        index=False)
