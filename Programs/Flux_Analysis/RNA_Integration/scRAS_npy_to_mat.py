...
scRAS_mat_location = Path("Data/scRNA/Processed/scRAS_mat/scRAS.mat")
mat_dict = {}
...
for filename in os.listdir(HR_model_location):
	...
	mat_dict["rxn"] = rxn_list

for npy_file in os.listdir(scRAS_npy_location):

	RAS_mat = np.load(scRAS_npy_location/npy_file)
	
	mdl_name = npy_file.split("-")[0][:2] + "-" + npy_file.split("-")[0][2:]
	model = flux_model_dict[mdl_name]
	print(mdl_name)
	print(RAS_mat)
	mat_dict[npy_file.replace("-","_").split(".")[0]] = RAS_mat

sio.savemat(scRAS_mat_location,mat_dict)
