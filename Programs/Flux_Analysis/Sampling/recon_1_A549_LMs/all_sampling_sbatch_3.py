...
while np.size(HR_samples) == 0:
	HR_samples = recon_flux_model.HRSampler_gene_bias_lincomb_pinch(warmup_points, 2000, 1,RPS,4,thermo_const={"prune_specific": ["biomass_reaction"], "NS_internal": Internal_nullspace_S},gene_penalty_mod = 9.7)
#HR_samples = flux_model.HRSampler_lincom(warmup_dict[flux_model_name], 10000, 500)
print(HR_samples)

np.save(output_path / (f'tg3_initial_{str(round(time.time()*1000))}'), HR_samples)
