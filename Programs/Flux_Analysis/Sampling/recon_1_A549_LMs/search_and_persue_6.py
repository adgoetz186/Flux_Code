...
new_LMs,error_term,samples = sampling_w_error(all_LMs,RPS_names,reaction_objectives,recon_flux_model,warmup_points,Internal_nullspace_S,alpha = 100)
...
while error_term > 0.05 and len(error_terms) <= 25:
	new_LMs, error_term,samples = sampling_w_error(new_LMs, RPS_names, reaction_objectives, recon_flux_model,
										   warmup_points, Internal_nullspace_S, alpha=100/(len(error_terms)+1)**1.1)
	all_LMs = np.vstack((all_LMs, new_LMs))
	print(all_LMs)
	print(error_term)
