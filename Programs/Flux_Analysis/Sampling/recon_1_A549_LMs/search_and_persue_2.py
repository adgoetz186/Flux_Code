...
def sampling_w_error(LMs,RPS_names,reaction_objectives,model,warmup_points,Internal_nullspace_S,alpha = 0.05):
	...
	new_LMs = []
	for name_ind in range(len(RPS_names)):
		objectives.append((reaction_objectives[name_ind] - np.average(HR_samples[499::100,rxn_list.index(RPS_names[name_ind])]))**2/reaction_objectives[name_ind]**2)
...
list_of_reactions_to_opt = ['EX_lac_L(e)']
dict_opt = {}
...
all_LMs = np.ones(len(RPS_names))*-100
new_LMs,error_term,samples = sampling_w_error(all_LMs,RPS_names,reaction_objectives,recon_flux_model,warmup_points,Internal_nullspace_S,alpha = 1)
all_LMs = np.vstack((all_LMs,new_LMs))
error_terms = [error_term]
while error_term > 0.05 and len(error_terms) <= 15:
	new_LMs, error_term,samples = sampling_w_error(new_LMs, RPS_names, reaction_objectives, recon_flux_model,
										   warmup_points, Internal_nullspace_S, alpha=1)
	all_LMs = np.vstack((all_LMs, new_LMs))
	print(all_LMs)
	print(error_term)
	error_terms.append(error_term)
	time.sleep(20)
np.save(output_path / (f'opt2_{str(round(time.time()*1000))}'), samples)
LM_ID = f'opt2_{str(round(time.time()*1000))}'
np.save(op_LM / LM_ID, all_LMs)
np.save(op_LM / ("Perf2_"+LM_ID), np.array(error_terms))
