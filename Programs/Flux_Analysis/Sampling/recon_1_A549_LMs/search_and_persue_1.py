...
all_LMs = np.ones(len(RPS_names))*-10
new_LMs,error_term,samples = sampling_w_error(all_LMs,RPS_names,reaction_objectives,recon_flux_model,warmup_points,Internal_nullspace_S,alpha = 2)
...
while error_term > 0.05 and len(error_terms) <= 15:
	new_LMs, error_term,samples = sampling_w_error(new_LMs, RPS_names, reaction_objectives, recon_flux_model,
										   warmup_points, Internal_nullspace_S, alpha=2)
	all_LMs = np.vstack((all_LMs, new_LMs))
	print(all_LMs)
	print(error_term)
	error_terms.append(error_term)
	time.sleep(20)
np.save(output_path / (f'opt1_{str(round(time.time()*1000))}'), samples)
LM_ID = f'opt1_{str(round(time.time()*1000))}'
np.save(op_LM / LM_ID, all_LMs)
np.save(op_LM / ("Perf1_"+LM_ID), np.array(error_terms))
