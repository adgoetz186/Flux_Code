...
print(pos_rxn_array)
internal_lb, internal_ub, internal_S, b, internal_rxn, met_list = flux_model_copy.generate_mat_wo_exchange(prune_specific=["biomass_reaction"])
...
print(np.alltrue(internal_S == test_bi_S))
input()
print(rxn_array)
count = np.arange(np.size(pos_rxn_array),dtype=float)
zeros = np.zeros(np.size(pos_rxn_array),dtype=float)
ones = np.ones(np.size(pos_rxn_array),dtype=float)
print()
cutter = np.nonzero(np.where(np.array([("_inverted" in i and i.replace("_inverted","") in pos_rxn_array) for i in pos_rxn_array]),zeros,ones))[0]
simp_neg_ind = np.where(np.array([("_inverted" in i and i.replace("_inverted","") not in pos_rxn_array) for i in pos_rxn_array]),-1*ones,ones)
print(pos_rxn_array)
print("pos",len(pos_rxn_array))
input()

comp_neg_ind = []
comp_perm = []
print(pos_rxn_list[913],pos_rxn_list[912])
for i in range(len(pos_rxn_list)):
	...
	if "_inverted" not in rxn_name and rxn_name + "_inverted" in pos_rxn_array:
		comp_neg_ind.append(0.0)
		comp_perm.append(pos_rxn_list.index(rxn_name + "_inverted"))
	elif "_inverted" in rxn_name and rxn_name.replace("_inverted","") in pos_rxn_array:
...
print(np.argmax(comp_neg_ind))
	# The final thing which must be done is replace all the -5 entries with indicies which are not indicies of inverted reactions
comp_perm = np.array(comp_perm)
comp_neg_ind = np.array(comp_neg_ind)
print(comp_perm)
print(comp_neg_ind)
print(comp_neg_ind[comp_perm])
timestart = time.time()
test_positive_point_to_test = copy.deepcopy(test_positive_point)
comp_neg_ind_to_test = copy.deepcopy(comp_neg_ind)
test_positive_point_to_test *= simp_neg_ind
comp_neg_ind_to_test *= test_positive_point_to_test
comp_neg_ind_to_test = comp_neg_ind_to_test[comp_perm]
test_positive_point_to_test += comp_neg_ind_to_test
test_positive_point_to_test = test_positive_point_to_test[cutter]
print("start")
print(timestart - time.time())
print("done")
...
input()
comp_neg_ind = np.where(np.array([("_inverted" in i and i.replace("_inverted","") in pos_rxn_array) for i in pos_rxn_array]),-1*ones,zeros)
comp_perm_array = np.nonzero(np.where(np.array([("_inverted" not in i and (i+"_inverted") in pos_rxn_array) for i in pos_rxn_array]),zeros,ones))[0]
print(cutter)
print(pos_rxn_list)
