...
for flux_model_name in flux_model_dict.keys():
	...
	if int(flux_model_name.split("-")[-1]) <= 2:
		flux_model = flux_model_dict[flux_model_name]
		# 10000
		# 5
		warmup = flux_model.generate_warmup_gb_pinch(10000,[1,1.005,1.01], repeat_basis= 5,points_save_path = warmup_sample_location,model_save_path = HR_ready_model_location,trans_NS = trans_NS,pinch_1_norm=1.1)
