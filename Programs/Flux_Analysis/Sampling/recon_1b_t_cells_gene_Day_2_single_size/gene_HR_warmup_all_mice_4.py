...
for flux_model_name in flux_model_dict.keys():
	...
	if (int(flux_model_name.split("-")[-1]) > 7) and (int(flux_model_name.split("-")[-1]) <= 8):
...
# why does setting the max flux to 1.05 fix the issue
# Its due to pinching, maybe see if there is a vector that gets pinched at 1.01 and not 1.05