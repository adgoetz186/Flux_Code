...
def folder_to_dtf(path_to_folder,rxns,start,skip):
	all_points = np.empty(0)
	for file in os.listdir(path_to_folder):
		if np.size(all_points) == 0:
			points = np.load(path_to_folder/Path(file))
			all_points = points[start::skip,:-1]
		else:
			points = np.load(path_to_folder/Path(file))[start::skip,:-1]
			all_points = np.vstack((all_points,points))
...
for condition_name in dataframe_list:
	if "B6" in condition_name and "therm" in condition_name:
		points_dtf = folder_to_dtf(f"Data/HR/HR_fin/{condition_name}", rxn_list,1600,100)
		B6_dtf_list.append(points_dtf.mean())
	if "TC" in condition_name and "therm" in condition_name:
		points_dtf = folder_to_dtf(f"Data/HR/HR_fin/{condition_name}",rxn_list,1600,100)
...
rxns_of_interest = ["PGI","PFK","G6PDH2r","PGK"]
