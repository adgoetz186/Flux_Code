...
for mouse_number in mouse_number_filename_dict.keys():
    ...
    Obj_array_no_val = model_dict["model_alignment_performances_no_valine"]
    ...
    best_ind = np.argmin(Obj_array_no_val)


    
    plt.scatter(1/np.array(alpha_list[best_ind]),Obj_array_no_val[best_ind])
    plt.plot(1/np.array(alpha_list),Obj_array_no_val,label = "V penalty removed")
