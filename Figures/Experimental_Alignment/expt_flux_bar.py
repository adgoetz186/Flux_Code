...
for mouse_number in mouse_number_filename_dict.keys():
    file_location = Path(f"Data/experimental_alignment_data/recon_1b_t_cells_gene_support/Individual_Mice/{mouse_number_filename_dict[mouse_number]}/experimental_alignment_result_data_b_Day_2_two_sizes.pkl")
    with open(file_location, "rb") as readfile:
        model_dict = pickle.load(readfile)
    model_file_location = Path("Data/Models/json_models/fast_key_format/recon_1b_t_cells/exp_aligned_Day_2_two_sizes/")
    ...
    Xlb_mdl = [recon2_2_rxn_added.model_dict["rxn_dict"][ordered_rxn[i]]['lb'] / alpha for i in ind]
    Xub_mdl = [recon2_2_rxn_added.model_dict["rxn_dict"][ordered_rxn[i]]['ub'] / alpha for i in ind]
    print(Xlb_mdl)
    print(plot_model)
    ...
    width = 0.25
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width / 2, plot_experiment, width, label='Expt')
    rects1 = ax.bar(x + width / 2, plot_model, width, label='Mdl')
