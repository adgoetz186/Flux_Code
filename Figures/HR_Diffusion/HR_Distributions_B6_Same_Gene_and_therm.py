...
plt.scatter(np.log10(np.abs(cd_list)), np.log10(np.mean(HR_Points_location_B6_1_A, axis=0) + np.mean(HR_Points_location_B6_1_B, axis=0)))
plt.show()
...
for i in cd_list_ind:
	...
	if not np.isnan(cd_list[i]):
		...
		plt.hist(HR_Points_location_B6_1_B[:, i], bins=np.linspace(np.min(np.hstack((HR_Points_location_B6_1_B[:, i], HR_Points_location_B6_1_A[:, i]))),
		                                          np.max(np.hstack((HR_Points_location_B6_1_B[:, i], HR_Points_location_B6_1_A[:, i]))), 50), alpha=0.5,
		         label="B6_1 first half")
		plt.hist(HR_Points_location_B6_1_A[:, i], bins=np.linspace(np.min(np.hstack((HR_Points_location_B6_1_B[:, i], HR_Points_location_B6_1_A[:, i]))),
		                                          np.max(np.hstack((HR_Points_location_B6_1_B[:, i], HR_Points_location_B6_1_A[:, i]))), 50), alpha=0.5,
