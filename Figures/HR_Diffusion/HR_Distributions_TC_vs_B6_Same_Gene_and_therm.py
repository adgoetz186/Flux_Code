...
HR_Points_location_B6_path = Path("Data/HR/recon_1b_t_cells_gene_Day_2_single_B6_1_gene_therm")
HR_Points_location_B6_5_all = np.array([])
...
for file in os.listdir(HR_Points_location_TC_5_path):
	...
	if np.size(HR_Points_location_TC_5_all) == 0:
		HR_Points_location_TC_5_all = points_for_distances
	else:
		HR_Points_location_TC_5_all = np.vstack((HR_Points_location_TC_5_all,points_for_distances))
len_total = np.shape(HR_Points_location_TC_5_all)
print(len_total[0]//2)
HR_Points_location_TC_5_A = HR_Points_location_TC_5_all



for file in os.listdir(HR_Points_location_B6_path):
	new_points = np.load(HR_Points_location_B6_path / file)
	points_for_distances = new_points[start:start+max_distance * values+1:max_distance]
	if np.size(HR_Points_location_B6_5_all) == 0:
		HR_Points_location_B6_5_all = points_for_distances
	else:
		HR_Points_location_B6_5_all = np.vstack((HR_Points_location_B6_5_all,points_for_distances))
len_total = np.shape(HR_Points_location_B6_5_all)
print(len_total[0]//2)
HR_Points_location_TC_5_B = HR_Points_location_B6_5_all



print(np.shape(HR_Points_location_TC_5_A))
print(np.shape(HR_Points_location_TC_5_B))
input()
...
plt.scatter(np.log10(np.abs(cd_list)), np.log10(np.mean(HR_Points_location_TC_5_A, axis=0) + np.mean(HR_Points_location_TC_5_B, axis=0)))
plt.show()
cd_list_ind = np.flip(np.argsort(np.abs(cd_list)))
print(cd_list)
print(np.size(cd_list))
print(len(rxn_list))
print(rxn_list[-1])
...
for i in cd_list_ind:
	...
	if not np.isnan(cd_list[i]):
		plt.title(rxn_list[i])
		plt.hist(HR_Points_location_TC_5_B[:, i], bins=np.linspace(np.min(np.hstack((HR_Points_location_TC_5_B[:, i], HR_Points_location_TC_5_A[:, i]))),
		                                          np.max(np.hstack((HR_Points_location_TC_5_B[:, i], HR_Points_location_TC_5_A[:, i]))), 50), alpha=0.5,
		         label="B6_1")
		plt.hist(HR_Points_location_TC_5_A[:, i], bins=np.linspace(np.min(np.hstack((HR_Points_location_TC_5_B[:, i], HR_Points_location_TC_5_A[:, i]))),
		                                          np.max(np.hstack((HR_Points_location_TC_5_B[:, i], HR_Points_location_TC_5_A[:, i]))), 50), alpha=0.5,
		         label='TC_5')
		#plt.legend()
		#plt.show()
